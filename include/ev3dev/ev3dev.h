...
//-----------------------------------------------------------------------------
// The engine class provides a uniform interface for using motors with
// positional and directional feedback such as the EV3 and NXT motors.
// This feedback allows for precise control of the motors. This is the
// most common type of engine, so we just call it `engine`.
// 
// The way to configure a engine is to set the '_sp' attributes when
...
class motor : protected device {
    public:
        ...
        static constexpr char motor_large[]  = "lego-ev3-l-engine";
        static constexpr char motor_medium[] = "lego-ev3-m-engine";
        static constexpr char motor_nxt[] = "lego-nxt-engine";

        using device::connected;
        using device::device_index;

        // Run the engine until another command is sent.
        ...
        // When the new position is reached, the engine will stop using
        // the action specified by `stop_action`.
        static constexpr char command_run_to_rel_pos[] = "run-to-rel-pos";

        // Run the engine for the amount of time specified in `time_sp`
        // and then stop the engine using the action specified by `stop_action`.
        static constexpr char command_run_timed[] = "run-timed";

        // Run the engine at the duty cycle specified by `duty_cycle_sp`.
        ...
        // Reset all of the engine parameter attributes to their default value.
        // This will also have the effect of stopping the engine.
        ...
        // cause the engine to rotate clockwise.
        static constexpr char polarity_normal[] = "normal";

        // With `inversed` polarity, a positive duty cycle will
        // cause the engine to rotate counter-clockwise.
        static constexpr char polarity_inversed[] = "inversed";

        // Power is being sent to the engine.
        static constexpr char state_running[] = "running";

        // The engine is ramping up or down and has not yet reached a constant output level.
        static constexpr char state_ramping[] = "ramping";

        // The engine is not turning, but rather attempting to hold a fixed position.
        static constexpr char state_holding[] = "holding";

        // The engine is turning, but cannot reach its `speed_sp`.
        static constexpr char state_overloaded[] = "overloaded";

        // The engine is not turning when it should be.
        static constexpr char state_stalled[] = "stalled";

        // Power will be removed from the engine and it will freely coast to a stop.
        static constexpr char stop_action_coast[] = "coast";

        // Power will be removed from the engine and a passive electrical load will
        // be placed on the engine. This is usually done by shorting the engine terminals
        // together. This load will absorb the energy from the rotation of the motors and
        // cause the engine to stop more quickly than coasting.
        static constexpr char stop_action_brake[] = "brake";

        // Does not remove power from the engine. Instead it actively try to hold the engine
        // at the current position. If an external force tries to turn the engine, the engine
        // will `push back` to maintain its position.
        static constexpr char stop_action_hold[] = "hold";

        // Address: read-only
        // Returns the name of the port that this engine is connected to.
        std::string address() const { return get_attr_string("address"); }

        // Command: write-only
        // Sends a command to the engine controller. See `commands` for a list of
        // possible values.
        ...
        // Commands: read-only
        // Returns a list of commands that are supported by the engine
        // controller. Possible values are `run-forever`, `run-to-abs-pos`, `run-to-rel-pos`,
        // `run-timed`, `run-direct`, `stop` and `reset`. Not all commands may be supported.
        // 
        // - `run-forever` will cause the engine to run until another command is sent.
        // - `run-to-abs-pos` will run to an absolute position specified by `position_sp`
        //   and then stop using the action specified in `stop_action`.
        // - `run-to-rel-pos` will run to a position relative to the current `position` value.
        //   The new position will be current `position` + `position_sp`. When the new
        //   position is reached, the engine will stop using the action specified by `stop_action`.
        // - `run-timed` will run the engine for the amount of time specified in `time_sp`
        //   and then stop the engine using the action specified by `stop_action`.
        // - `run-direct` will run the engine at the duty cycle specified by `duty_cycle_sp`.
        //   Unlike other run commands, changing `duty_cycle_sp` while running *will*
        //   take effect immediately.
        // - `stop` will stop any of the run commands before they are complete using the
        //   action specified by `stop_action`.
        // - `reset` will reset all of the engine parameter attributes to their default value.
        //   This will also have the effect of stopping the engine.
        mode_set commands() const { return get_attr_set("commands"); }

        // Count Per Rot: read-only
        // Returns the number of tacho counts in one rotation of the engine. Tacho counts
        ...
        // Returns the number of tacho counts in one meter of travel of the engine. Tacho
        ...
        // Driver Name: read-only
        // Returns the name of the driver that provides this tacho engine device.
        std::string driver_name() const { return get_attr_string("driver_name"); }

        // Duty Cycle: read-only
        // Returns the current duty cycle of the engine. Units are percent. Values
        // are -100 to 100.
        ...
        // the engine to rotate in reverse.
        ...
        // Returns the number of tacho counts in the full travel of the engine. When
        // combined with the `count_per_m` atribute, you can use this value to
        // calculate the maximum travel distance of the engine. (linear motors only)
        int full_travel_count() const { return get_attr_int("full_travel_count"); }

        // Polarity: read/write
        // Sets the polarity of the engine. With `normal` polarity, a positive duty
        // cycle will cause the engine to rotate clockwise. With `inversed` polarity,
        // a positive duty cycle will cause the engine to rotate counter-clockwise.
        ...
        // Position: read/write
        // Returns the current position of the engine in pulses of the rotary
        // encoder. When the engine rotates clockwise, the position will increase.
        ...
        // may be slightly different than the maximum speed that a particular engine can
        // reach - it's the maximum theoretical speed.
        int max_speed() const { return get_attr_int("max_speed"); }

        // Speed: read-only
        // Returns the current engine speed in tacho counts per second. Note, this is
        ...
        // value causes the engine to rotate in reverse with the exception of `run-to-*-pos`
        ...
        // engine speed will increase from 0 to 100% of `max_speed` over the span of this
        ...
        // are in milliseconds and must be positive. When set to a non-zero value, the
        // engine speed will decrease from 0 to 100% of `max_speed` over the span of this
        // setpoint. The actual ramp time is the ratio of the difference between the
        ...
        // Returns a list of stop actions supported by the engine controller.
        // Possible values are `coast`, `brake` and `hold`. `coast` means that power will
        // be removed from the engine and it will freely coast to a stop. `brake` means
        // that power will be removed from the engine and a passive electrical load will
        // be placed on the engine. This is usually done by shorting the engine terminals
        // together. This load will absorb the energy from the rotation of the motors and
        // cause the engine to stop more quickly than coasting. `hold` does not remove
        // power from the engine. Instead it actively tries to hold the engine at the current
        // position. If an external force tries to turn the engine, the engine will 'push
        // back' to maintain its position.
        mode_set stop_actions() const { return get_attr_set("stop_actions"); }

        // Time SP: read/write
        // Writing specifies the amount of time the engine will run when using the
...
//-----------------------------------------------------------------------------
// The DC engine class provides a uniform interface for using regular DC motors
...
class dc_motor : protected device {
    public:
        ...
        // Command: write-only
        // Sets the command for the engine. Possible values are `run-forever`, `run-timed` and
        ...
        // Commands: read-only
        // Returns a list of commands supported by the engine
        // controller.
        mode_set commands() const { return get_attr_set("commands"); }

        // Driver Name: read-only
        // Returns the name of the engine driver that loaded this device. See the list
        // of [supported devices] for a list of drivers.
        std::string driver_name() const { return get_attr_string("driver_name"); }

        // Duty Cycle: read-only
        // Shows the current duty cycle of the PWM signal sent to the engine. Values
        // are -100 to 100 (-100% to 100%).
        int duty_cycle() const { return get_attr_int("duty_cycle"); }

        // Duty Cycle SP: read/write
        // Writing sets the duty cycle setpoint of the PWM signal sent to the engine.
        ...
        // Polarity: read/write
        // Sets the polarity of the engine. Valid values are `normal` and `inversed`.
        std::string polarity() const { return get_attr_string("polarity"); }
        ...
        // Ramp Down SP: read/write
        // Sets the time in milliseconds that it take the engine to ramp down from 100%
        ...
        // Ramp Up SP: read/write
        // Sets the time in milliseconds that it take the engine to up ramp from 0% to
        ...
        // State: read-only
        // Gets a list of flags indicating the engine status. Possible
        // flags are `running` and `ramping`. `running` indicates that the engine is
        // powered. `ramping` indicates that the engine has not yet reached the
        // `duty_cycle_sp`.
        mode_set state() const { return get_attr_set("state"); }

        // Stop Action: write-only
        // Sets the stop action that will be used when the engine stops. Read
...
//-----------------------------------------------------------------------------
// The servo engine class provides a uniform interface for using hobby type
// servo motors.
//-----------------------------------------------------------------------------
class servo_motor : protected device {
    public:
        ...
        // `position_sp` attribute. Setting to `float` will remove power from the engine.
        ...
        // where the engine does not turn. You must write to the position_sp attribute for
        // changes to this attribute to take effect.
        ...
        // Possible values are:
        // * `running`: Indicates that the engine is powered.
        mode_set state() const { return get_attr_set("state"); }
...
// or engine is connected. In some cases though, this must be manually specified
...
class lego_port : protected device {
    public:
        ...
        // Generally speaking when the mode changes any sensor or engine devices
